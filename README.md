...
There are two possibilities for the graphs that are generated:
  1. The effect of number of threads on latency and throughput, for fixed vector sizes (this is the default). Usage: `gengraphs.py FILE` or optionally `gengraphs.py FILE [threads]`, where the threads switch is redundant.
